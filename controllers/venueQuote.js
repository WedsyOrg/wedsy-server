...
const quotePdf = async (req, res) => {
  try {
    const venue = await resolveOwnedVenue(req, res, "name address formattedAddress contact phone email logo tagline gstin pan settings bankDetails spaces");
    if (!venue) return;
    const quote = await VenueQuote.findOne({ _id: req.params.quoteId, venue: venue._id }).lean();
    if (!quote) return res.status(404).json({ message: "Quote not found" });
    // Scoped like the other enquiry reads: never render a lead the requester
    // cannot see (or a soft-deleted one) into a PDF. functions carry the
    // quoted spaces; contacts carry the client party.
    const enquiry = await resolveScopedEnquiry(req.venueOwner, req.venueMember, venue._id, quote.enquiry, { select: "coupleName name couplePhone checkIn checkOut requirements functions contacts", lean: true });
...
module.exports = { createQuote, listQuotes, getQuote, updateQuote, confirmBookingFromQuote, quotePdf, normalizeQuoteLines, applyQuoteToBooking, storedLineMode };
